...
public class TenantController : BaseController
{
	...
	public async Task<ActionResult<bool>> HasActiveTenant(int propertyId)
	{
		try
		{
			var hasActiveTenant = await _tenantService.HasActiveTenantAsync(propertyId);
	...
	public async Task<ActionResult<decimal>> GetCurrentMonthlyRent(int tenantId)
	{
		try
		{
			var monthlyRent = await _tenantService.GetCurrentMonthlyRentAsync(tenantId);
	...
	public async Task<ActionResult<bool>> IsLeaseExpiringInDays(int tenantId, int days)
	{
		try
		{
			var isExpiring = await _tenantService.IsLeaseExpiringInDaysAsync(tenantId, days);
	...
	public async Task<ActionResult<List<TenantResponse>>> GetTenantsWithExpiringLeases(int daysAhead)
	{
		try
		{
			...
			var expiringTenants = await _tenantService.GetTenantsWithExpiringLeasesAsync(landlordId.Value, daysAhead);
