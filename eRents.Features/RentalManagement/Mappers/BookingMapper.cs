...
public static class BookingMapper
{
	...
	public static IQueryable<Booking> ApplySearchFilters(this IQueryable<Booking> query, BookingSearchObject search)
	{
		...
		// Use the enum Status property from the base class
		var baseSearch = search as AdvancedBookingSearch;
		if (baseSearch?.Status.HasValue == true)
			query = query.Where(b => b.Status == baseSearch.Status.Value);

		// Filter by generic status string
		if (!string.IsNullOrEmpty(search.GenericStatusString))
		{
			if (Enum.TryParse<BookingStatusEnum>(search.GenericStatusString, true, out var statusEnum))
				query = query.Where(b => b.Status == statusEnum);
		}

		// Filter by BookingStatusName (from BasicBookingSearch)
		if (!string.IsNullOrEmpty(search.BookingStatusName))
		{
			if (Enum.TryParse<BookingStatusEnum>(search.BookingStatusName, true, out var statusEnum))
				query = query.Where(b => b.Status == statusEnum);
		}
		...
		if (!search.IncludeCancelled)
			query = query.Where(b => b.Status != BookingStatusEnum.Cancelled);
	...
	public static IQueryable<Booking> ApplySorting(this IQueryable<Booking> query, string? sortBy, bool sortDescending = false)
	{
		...
		return sortBy.ToLower() switch
		{
			...
			"bookingstatus" or "status" => sortDescending ? query.OrderByDescending(b => b.Status) : query.OrderBy(b => b.Status),
