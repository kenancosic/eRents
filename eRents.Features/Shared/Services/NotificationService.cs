...
namespace eRents.Features.Shared.Services
{
    ...
    public class NotificationService : INotificationService
    {
        ...
        public async Task CreateBookingNotificationAsync(int userId, int bookingId, string title, string message)
        {
            ...
            // Also send email notification for booking events
        ...
        public async Task SendEmailNotificationAsync(int userId, string subject, string message)
        {
            try
            {
                ...
                // Publish to RabbitMQ email queue for async processing via IEmailService
                if (_emailService != null)
                {
                    ...
                    _logger.LogInformation("📧 Email queued for User {UserId} ({Email}): {Subject}",
                            userId, user.Email, subject);
                }
                else
                {
                    // Fallback: Log if email service is not available
                    _logger.LogWarning("Email service not available - Email not sent to User {UserId} ({Email}): {Subject}",
                            userId, user.Email, subject);
                }
            }
            catch (Exception ex)
